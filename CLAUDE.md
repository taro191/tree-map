...
- `server/db.js` = `createPgStore(connectionString)` คุย Postgres จริงผ่าน `pg` (parameterized query, upsert ด้วย `ON CONFLICT`) แปลง row snake_case ↔ object camelCase ให้ตรงกับ shape เดิมที่ client ใช้อยู่แล้ว มีทั้ง CRUD ของ plots/trees และ users (`createUser`, `findUserByEmail`, `findUserById`)
