...
export default function EditUserModal({ user, entities, onClose, onSave }) {
  const [name, setName] = useState(user.name || '');
  const [email, setEmail] = useState(user.email || '');
  const [phone, setPhone] = useState(user.phone || '');
  const [role, setRole] = useState(user.role);
  const [managedCommunityEnterpriseId, setManagedCommunityEnterpriseId] = useState(user.managedCommunityEnterpriseId || '');
  ...
  async function save() {
    ...
    try {
      await onSave({
        name: name.trim() || null, email: email.trim() || null, phone: phone.trim() || null,
        role, managedCommunityEnterpriseId: role === 'enterprise_admin' ? managedCommunityEnterpriseId : null
      });
      onClose();
    } catch (err) {
    ...
    <div className="fixed inset-0 z-30 flex items-center justify-center bg-black/40 p-4" onClick={onClose}>
      <div className="w-full max-w-md rounded border border-gray-200 bg-white shadow-lg" onClick={e => e.stopPropagation()}>
        <div className="border-b border-gray-200 bg-gray-50 px-4 py-2.5">
          <h3 className="text-sm font-bold text-slate-700">แก้ไขผู้ใช้</h3>
        </div>
        <div className="space-y-3 p-4">
          ...
          <div>
            ...
            <input value={name} onChange={e => setName(e.target.value)} className="w-full rounded border border-gray-300 px-2 py-1.5 text-sm focus:border-emerald-600 focus:outline-none" />
          </div>
          <div>
