...
export default function MapView({ plots, trees, selectedPlotId, onSelectPlot, onMapReady, zoomControl = true, zoomSnap = 1 }) {
  const bounds = useMemo(() => computeBounds(plots, trees), [plots, trees]);

  return (
    <MapContainer center={[13.7563, 100.5018]} zoom={6} maxZoom={21} zoomControl={zoomControl} zoomSnap={zoomSnap} className="h-full w-full">
      <TileLayer
